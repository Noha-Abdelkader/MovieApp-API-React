...
{error? error.map((msg, index) =>{ return<>
  {(msg.message.includes("userName"))? 
 <p className='text-danger' key={index}>{msg.message}</p>:''}
</> }) :''} 



  <input className="inputStyle" type="number" placeholder='Age' onChange={(e)=> userData(e)} />
  {error? error.map((msg, index)=>{
    return <>
     {(msg.message.includes('Age')) ? <p className='text-danger' key={index}>{msg.message}</p>:''}
    </>}):''}

 

<input className="inputStyle" type="email" placeholder='Email' onChange={ (e)=>userData(e)}/>
{error? error.map((msg, index)=>{
  return<>
  {(msg.message.includes('Email'))?<p className='text-danger' key={index}>{msg.message},ex: example@mail.com </p>:''}
  </>}):''}
<input className="inputStyle" type="password" placeholder='Password' onChange={ (e)=>userData(e)}/>
{error? error.map((msg, index)=>{
  return<>
   {(msg.message.includes('Password'))? <p className='text-danger' key={index}>Invalid-password, password must contain at least 5 characters or numbers</p>:''}
  </>
  }):''}
 </div>
